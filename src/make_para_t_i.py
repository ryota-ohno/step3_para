...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):##R3:t-shaped R4:slipped-parallel
    ...
    monomer_array_i02 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],-A1,-A2,-A3, phi1)
    if a_ > b_:
        monomer_array_ip3 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2]+R4,-A1,-A2,-A3, phi1)
        monomer_array_ip4 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2]-R4,-A1,-A2,-A3, phi1)
    else:
        monomer_array_ip3 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2]+2*R3-R4,-A1,-A2,-A3, phi1)
        monomer_array_ip4 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2]-(2*R3-R4),-A1,-A2,-A3, phi1)
...
def get_file_name_from_dict(monomer_name,paras_dict):
    ...
    for key,val in paras_dict.items():
        if key in ['a','b','cx','cy','cz','theta','R3','R4','A1']:
            val = np.round(val,2)
