...
def make_gjf_xyz(auto_dir,monomer_name,params_dict,machine_type,isInterlayer):##R3:t-shaped R4:slipped-parallel
    a_ = params_dict['a']; b_ = params_dict['b']; c = np.array([params_dict['cx'],params_dict['cy'],params_dict['cz']])
    R3 = params_dict['R3']; R4 = params_dict['R4']; A2 = params_dict['A2']; A3 = params_dict['theta']
    phi1 = params_dict.get('phi1',0.0); phi2 = params_dict.get('phi2',0.0)
    print(phi1, phi2)
    ##平行配置をA2傾けてT字を-A2傾ける　この時A2は負の値をとる
    monomer_array_c1 = get_monomer_xyzR(monomer_name,0,0,0,0,A2,A3, phi1)##centerのc
    
    if a_ > b_:
        monomer_array_p1 = get_monomer_xyzR(monomer_name,0,b_,R4,0,A2,A3, phi1)##p1がb方向
        monomer_array_ip1 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2]+R4,0,A2,A3, phi1)
        monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2]-R4,0,A2,A3, phi1)
    else:##a_<b_
        monomer_array_p1 = get_monomer_xyzR(monomer_name,a_,0,2*R3-R4,0,A2,A3, phi1)##p2がa方向
        monomer_array_ip1 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2]+2*R3-R4,0,A2,A3, phi1)
        monomer_array_ip2 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2]-(2*R3-R4),0,A2,A3, phi1)
    
    monomer_array_c2 = get_monomer_xyzR(monomer_name,0,0,0,0,-A2,-A3, phi1)##centerのc 角度-A3
    monomer_array_i02 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],0,-A2,-A3, phi1)
    if a_ > b_:
        monomer_array_ip3 = get_monomer_xyzR(monomer_name,c[0],c[1]+b_,c[2]+R4,0,-A2,-A3, phi1)
        monomer_array_ip4 = get_monomer_xyzR(monomer_name,c[0],c[1]-b_,c[2]-R4,0,-A2,-A3, phi1)
    else:
        monomer_array_ip3 = get_monomer_xyzR(monomer_name,c[0]+a_,c[1],c[2]+2*R3-R4,0,-A2,-A3, phi1)
        monomer_array_ip4 = get_monomer_xyzR(monomer_name,c[0]-a_,c[1],c[2]-(2*R3-R4),0,-A2,-A3, phi1)

    monomer_array_i01 = get_monomer_xyzR(monomer_name,c[0],c[1],c[2],0,A2,A3, phi1)
    monomer_array_t1 = get_monomer_xyzR(monomer_name,a_/2,b_/2,R3,0,-A2,-A3, phi2)
    monomer_array_t2 = get_monomer_xyzR(monomer_name,a_/2,-b_/2,R3-R4,0,-A2,-A3, phi2)
    monomer_array_t3 = get_monomer_xyzR(monomer_name,-a_/2,-b_/2,-R3,0,-A2,-A3, phi2)
    monomer_array_t4 = get_monomer_xyzR(monomer_name,-a_/2,b_/2,-R3+R4,0,-A2,-A3, phi2)
    monomer_array_it1 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]+b_/2,c[2]+R3,0,-A2,-A3, phi2)
    monomer_array_it2 = get_monomer_xyzR(monomer_name,c[0]+a_/2,c[1]-b_/2,c[2]+R3-R4,0,-A2,-A3, phi2)
    monomer_array_it3 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]-b_/2,c[2]-R3,0,-A2,-A3, phi2)
    monomer_array_it4 = get_monomer_xyzR(monomer_name,c[0]-a_/2,c[1]+b_/2,c[2]-R3+R4,0,-A2,-A3, phi2)
...
def get_file_name_from_dict(monomer_name,paras_dict):
    ...
    for key,val in paras_dict.items():
        if key in ['a','b','cx','cy','cz','theta','R3','R4','A2']:
            val = np.round(val,2)
