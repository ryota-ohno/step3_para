...
##step3 精密化　tetracene pentaceneパターン　a,cx,czを最適化 12分子計算 ねじれA2有り
import os
...
from make_para_6 import exec_gjf
from step3_para_vdw_1 import get_c_vec_vdw
from step3_para_vdw_1 import detect_peaks
...
def init_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','R3','R4','E','E_t1','E_t2','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
    else:
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','cx','cz','A2','cy','b','theta','R3','R4','E','E_p','E_t1','E_t2','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['A2','b','theta','R3','R4','cy']
    opt_param_keys = ['a','cx','cz']

    auto_step1_csv = '/home/ohno/Working/step3_para/{}/step1_para_twist/step1.csv'.format(monomer_name)##これは何？
    df_step2 = pd.read_csv(auto_step1_csv)

    auto_csv = os.path.join(auto_dir,'step3_para.csv')
    df_E = pd.read_csv(auto_csv)
    df_queue = df_E.loc[df_E['status']=='InProgress',['machine_type','file_name','A2','a','b','theta','R3','R4']]
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        machine_type,file_name,A2,a,b,theta,R3,R4 = row
        ...
        if len(E_list)!=10:##計算する層状の分子数
            ...
            try:
                Ep, Et1, Et2 = df_step2[(df_step2['A2']==A2)&(df_step2['theta']==theta)&(df_step2['a']==a)&(df_step2['b']==b)][['E_p','E_t1','E_t2']].values[0]
            except IndexError:
                inner_params_dict = {"A2":A2,"a":a,"b":b,"theta":theta,'R3':R3,'R4':R4,'cx':0,'cy':0,'cz':0}
                ...
                while not(is_inner_over):
                    ...
                    is_inner_over = len(E_inner_list)==3
                Et1, Et2,Ep =map(float,E_inner_list)
                df_newline = pd.Series({**inner_params_dict,'E':2*(Ep+Et1+Et2),'E_p':Ep,'E_t1':Et1,'E_t2':Et2,'machine_type':machine_type,'status':'Done','file_name':inner_file_name})
                df_step2=df_step2.append(df_newline,ignore_index=True)
                df_step2.to_csv(auto_step1_csv,index=False)
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_p':0,'E_t1':0.,'E_t2':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'E_i02':0.,'E_ip3':0.,'E_ip4':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; cx_init_prev = init_params_dict['cx']; cz_init_prev = init_params_dict['cz']; A2 = init_params_dict['A2']
    
    while True:
        E_list=[];params_list=[]
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:##刻み幅は適宜変化させる 今回は0.1刻み
            for cx in [cx_init_prev-0.1,cx_init_prev,cx_init_prev+0.1]:
                 for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:  
                    a = np.round(a,1);cx = np.round(cx,1);cz= np.round(cz,1)
                    df_val_params = df_val[
                    (df_val['a']==a)&(df_val['cx']==cx)&(df_val['cz']==cz)&(df_val['A2']==A2)
                    &(df_val['status']=='Done')
                       ]
                    if len(df_val_params)==0:
                         return False,{'a':a,'cx':cx,'cz':cz}
                    params_list.append([a,cx,cz]);E_list.append(df_val_params['E'].values[0])
        a_init,cx_init,cz_init = params_list[np.argmin(np.array(E_list))]
        if a_init==a_init_prev and cx_init==cx_init_prev and cz_init==cz_init_prev:
            return True,{'a':a_init,'cx':cx_init, 'cz':cz_init}
        else:
            a_init_prev=a_init;cx_init_prev=cx_init;cz_init_prev=cz_init
