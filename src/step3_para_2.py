...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['a','theta','cz','cx','cy','b','R3','R4','E','E_p','E_t1','E_t2','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['b','R3','R4','cx','cy']
    opt_param_keys = ['a','theta','cz']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; theta_init_prev = init_params_dict['theta']; cz_init_prev = init_params_dict['cz']
        ...
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:##刻み幅は適宜変化させる 今回は0.1刻み
            for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
                 for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:  
                    a = np.round(a,1);theta = np.round(theta,1);cz= np.round(cz,1)
                    df_val_params = df_val[
                    (df_val['a']==a)&(df_val['theta']==theta)&(df_val['cz']==cz)
                    &(df_val['status']=='Done')
                       ]
                    if len(df_val_params)==0:
                         return False,{'a':a,'theta':theta,'cz':cz}
                    params_list.append([a,theta,cz]);E_list.append(df_val_params['E'].values[0])
        a_init,theta_init,cz_init = params_list[np.argmin(np.array(E_list))]
        if a_init==a_init_prev and theta_init==theta_init_prev and cz_init==cz_init_prev:
            return True,{'a':a_init,'theta':theta_init, 'cz':cz_init}
        else:
             a_init_prev=a_init;theta_init_prev=theta_init;cz_init_prev=cz_init
