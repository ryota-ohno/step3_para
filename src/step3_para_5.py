...
##step3 精密化　tetracene pentaceneパターン　a,cx,czを最適化 12分子計算
import os
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['b','theta','R3','R4','cy']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=12:##計算する層状の分子数
            continue
        else:##エネルギーの内訳全般
            len_queue-=1;machine_type_list.remove(machine_type)
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Eit1=float(E_list[3]);Eit2=float(E_list[4]);Eit3=float(E_list[5]);Eit4=float(E_list[6]);Ei02=float(E_list[7]);Eip3=float(E_list[8]);Eip4=float(E_list[9]);Et1=float(E_list[10]);Et2=float(E_list[11])##ここも計算する分子数に合わせて調整
            E = 2*((Ei01+Eip1+Eip2+Ei02+Eip3+Eip4)/2+Eit1+Eit2+Eit3+Eit4)+2*(Et1+Et2)##隣接20分子　2パターン
            #### TODO
            df_E.loc[idx, ['E_t1','E_t2','E_i01','E_ip1','E_ip2','E_it1','E_it2','E_it3','E_it4','E_i02','E_ip3','E_ip4','E','status']] = [Et1,Et2,Ei01,Eip1,Eip2,Eit1,Eit2,Eit3,Eit4,Ei02,Eip3,Eip4,E,'Done']
    ...
    if isAvailable:
        ...
        if len(params_dict)!=0:#終わりがまだ見えないなら
            ...
            if not(alreadyCalculated):
                ...
                df_newline = pd.Series({**params_dict,'E':0.,'E_t1':0.,'E_t2':0.,'E_i01':0.,'E_ip1':0.,'E_ip2':0.,'E_it1':0.,'E_it2':0.,'E_it3':0.,'E_it4':0.,'E_i02':0.,'E_ip3':0.,'E_ip4':0.,'machine_type':machine_type,'status':'InProgress','file_name':file_name})
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    a_init_prev = init_params_dict['a']; cx_init_prev = init_params_dict['cx']; cz_init_prev = init_params_dict['cz']
        ...
        for a in [a_init_prev-0.1,a_init_prev,a_init_prev+0.1]:##刻み幅は適宜変化させる 今回は0.1刻み
            for cx in [cx_init_prev-0.1,cx_init_prev,cx_init_prev+0.1]:
                 for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:  
                    ...
                    (df_val['a']==a)&(df_val['cx']==cx)&(df_val['cz']==cz)
                    &(df_val['status']=='Done')
                       ]
