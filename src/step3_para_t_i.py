...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','A1','A2','a','b','theta','R3','R4','E','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['a','b','theta','R3','R4','A1','A2']
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    cx_init_prev = init_params_dict['cx']; cy_init_prev = init_params_dict['cy']; cz_init_prev = init_params_dict['cz']; A1 = init_params_dict['A1']
    
    while True:
        E_list=[];cxyz_list=[]
        for cx in [cx_init_prev-0.1,cx_init_prev,cx_init_prev+0.1]:##刻み幅は適宜変化させる
            for cy in [cy_init_prev-0.1,cy_init_prev,cy_init_prev+0.1]:
                 for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:  
                    ...
                    df_val_cxyz = df_val[
                    (df_val['cx']==cx)&(df_val['cy']==cy)&(df_val['cz']==cz)&(df_val['A1']==A1)
                    &(df_val['status']=='Done')
                       ]
