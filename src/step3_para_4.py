...
##step3 精密化　tetracene pentaceneパターン　b,θ,czを最適化 13分子計算
import os
...
from make_para_2 import exec_gjf
...
def init_process(args):
    ...
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['cx','cy','cz','a','b','theta','R3','R4','E','E_p','E_t1','E_t2','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
    else:
...
def main_process(args):
    ...
    auto_csv_path = os.path.join(auto_dir,'step3_para.csv')
    if not os.path.exists(auto_csv_path):        
        df_E = pd.DataFrame(columns = ['b','theta','cz','cx','cy','a','R3','R4','E','E_p','E_t1','E_t2','E_i01','E_i02','E_ip1','E_ip2','E_ip3','E_ip4','E_it1','E_it2','E_it3','E_it4','machine_type','status','file_name'])##いじる
...
def listen(args):
    ...
    #### TODO
    fixed_param_keys = ['a','R3','R4','cx','cy']
    opt_param_keys = ['b','theta','cz']
    ...
    for idx,row in zip(df_queue.index,df_queue.values):
        ...
        if len(E_list)!=13:##計算する層状の分子数
            continue
        else:##エネルギーの内訳全般
            len_queue-=1;machine_type_list.remove(machine_type)
            Ei01=float(E_list[0]);Eip1=float(E_list[1]);Eip2=float(E_list[2]);Eit1=float(E_list[3]);Eit2=float(E_list[4]);Eit3=float(E_list[5]);Eit4=float(E_list[6]);Ei02=float(E_list[7]);Eip3=float(E_list[8]);Eip4=float(E_list[9]);Ep=float(E_list[10]);Et1=float(E_list[11]);Et2=float(E_list[12])##ここも計算する分子数に合わせて調整
...
def get_opt_params_dict(df_cur, init_params_dict,fixed_params_dict):
    ...
    b_init_prev = init_params_dict['b']; theta_init_prev = init_params_dict['theta']; cz_init_prev = init_params_dict['cz']
    
    while True:
        E_list=[];params_list=[]
        for b in [b_init_prev-0.1,b_init_prev,b_init_prev+0.1]:##刻み幅は適宜変化させる 今回は0.1刻み
            for theta in [theta_init_prev-0.5,theta_init_prev,theta_init_prev+0.5]:
                 for cz in [cz_init_prev-0.1,cz_init_prev,cz_init_prev+0.1]:  
                    b = np.round(b,1);theta = np.round(theta,1);cz= np.round(cz,1)
                    df_val_params = df_val[
                    (df_val['b']==b)&(df_val['theta']==theta)&(df_val['cz']==cz)
                    &(df_val['status']=='Done')
                       ]
                    if len(df_val_params)==0:
                         return False,{'b':b,'theta':theta,'cz':cz}
                    params_list.append([b,theta,cz]);E_list.append(df_val_params['E'].values[0])
        b_init,theta_init,cz_init = params_list[np.argmin(np.array(E_list))]
        if b_init==b_init_prev and theta_init==theta_init_prev and cz_init==cz_init_prev:
            return True,{'b':b_init,'theta':theta_init, 'cz':cz_init}
        else:
            b_init_prev=b_init;theta_init_prev=theta_init;cz_init_prev=cz_init
